...
function useState(initState){
	...
	if( states.length === currentStateCount ){  // 값이 없을 때 값 추가
	...
	const setState = function(newState){ 
		...
		else if(JSON.stringify(currentState) === JSON.stringify(newState)) return; // 객체가 동일하면 멈춤
		states[idx] = newState;
		update();
	}
	currentStateCount++;
...
function update(){ // dom 업데이트 할 때 사용하면 함수
...
function render(node,container){ 
	...
	debounceTimeout = setTimeout(()=>{ // setimout 을 debounceTimeout 에 집어넣음
